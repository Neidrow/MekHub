...
const Appointments: React.FC<AppointmentsProps> = ({ 
  ...
  const timelineDays = useMemo(() => {
    ...
    return Array.from({ length: daysInMonth }, (_, i) => {
      ...
      return { dateStr, dayName: d.toLocaleDateString('fr-FR', { weekday: 'short' }), dayNum: d.getDate(), month: d.toLocaleDateString('fr-FR', { month: 'short' }) };
    });
  }, [viewDate]);
  ...
  const confirmDelete = async () => { if (!appointmentToDelete) return; setDeleteLoading(true); try { await onDelete(appointmentToDelete.id); setAppointmentToDelete(null); } catch (error) { alert("Impossible de supprimer ce rendez-vous."); } finally { setDeleteLoading(false); } };
    ...
    <div className="space-y-8 animate-in fade-in duration-500 min-h-[80vh] flex flex-col">
      {appointmentToDelete && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4" onClick={() => setAppointmentToDelete(null)}>
          <div className="bg-white dark:bg-slate-900 rounded-[2rem] w-full max-w-sm p-8 shadow-2xl relative animate-in zoom-in duration-300 flex flex-col border dark:border-slate-800" onClick={(e) => e.stopPropagation()}>
             ...
             <p className="text-slate-500 dark:text-slate-400 text-center text-sm mb-8 leading-relaxed">Attention, cette action est <span className="font-bold text-rose-600 dark:text-rose-400">irréversible</span>. Le rendez-vous pour <span className="font-bold text-slate-700 dark:text-slate-300">{appointmentToDelete.type_intervention}</span> sera supprimé.</p>
             <div className="flex flex-col gap-3"><button onClick={confirmDelete} disabled={deleteLoading} className="w-full py-4 bg-rose-600 text-white font-black rounded-2xl hover:bg-rose-700 shadow-xl shadow-rose-600/20 transition-all uppercase tracking-widest text-xs flex items-center justify-center gap-2">{deleteLoading ? (<div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></div>) : "Supprimer définitivement"}</button><button onClick={() => setAppointmentToDelete(null)} disabled={deleteLoading} className="w-full py-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 font-black rounded-2xl hover:bg-slate-50 dark:hover:bg-slate-700 transition-all uppercase tracking-widest text-xs">Annuler</button></div>
          </div>
        </div>
      )}
      ...
      <div className="space-y-4">
        <div id="tour-calendar-nav" className="flex items-center justify-between px-2">
           ...
           <button onClick={handleToday} className="px-4 py-2 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-xs font-black uppercase tracking-widest rounded-xl hover:bg-slate-200 dark:hover:bg-slate-700 transition-all">Aujourd'hui</button>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 flex-1">
        {filteredAppointments.length === 0 ? (
          ...
          filteredAppointments.map((app) => {
            ...
            return (<div key={app.id} className="bg-white dark:bg-slate-900 p-6 lg:p-8 rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-slate-800 hover:shadow-xl transition-all group flex flex-col h-full relative animate-in zoom-in duration-300"><div className="absolute top-6 right-6 flex items-center gap-1.5 px-3 py-1 bg-slate-50 dark:bg-slate-800 rounded-full border border-slate-100 dark:border-slate-700"><div className={`w-1.5 h-1.5 rounded-full ${isSynced ? 'bg-emerald-500 animate-pulse' : 'bg-slate-300 dark:bg-slate-600'}`}></div><span className={`text-[8px] font-black uppercase tracking-widest ${isSynced ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-400 dark:text-slate-500'}`}>{isSynced ? 'Google' : 'Local'}</span></div><div className="flex items-center gap-4 mb-6"><div className="p-4 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-2xl font-black text-center min-w-[4.5rem] shadow-lg shadow-slate-200 dark:shadow-none"><span className="block text-lg leading-none">{app.heure.split(':')[0]}</span><span className="block text-xs opacity-60">:{app.heure.split(':')[1]}</span></div><div><span className={`inline-block px-3 py-1 rounded-xl text-[9px] font-black uppercase tracking-widest mb-1 ${getStatusStyle(app.statut)}`}>{statusLabels[app.statut]}</span><h4 className="font-black text-slate-800 dark:text-white line-clamp-1">{app.type_intervention}</h4></div></div><div className="space-y-3 mb-6 flex-grow"><div className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700"><div className="w-10 h-10 rounded-xl bg-white dark:bg-slate-700 flex items-center justify-center text-slate-400 dark:text-slate-300 font-black text-xs shadow-sm">{customer?.nom?.charAt(0) || '?'}</div><div className="min-w-0"><p className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase leading-none mb-1 tracking-tighter">Client</p><p className="text-sm font-bold text-slate-700 dark:text-slate-200 truncate">{customer?.nom} {customer?.prenom}</p></div></div>{vehicle && (<div className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700"><div className="w-10 h-10 rounded-xl bg-white dark:bg-slate-700 flex items-center justify-center text-slate-400 dark:text-slate-300 font-black text-xs shadow-sm"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 17h2c.6 0 1-.4 1-1v-3c0-.9-.7-1.7-1.5-1.9C18.7 10.6 16 10 16 10s-1.3-1.4-2.2-2.3c-.5-.4-1.1-.7-1.8-.7H5c-.6 0-1.1.4-1.4.9l-1.4 2.9C2.1 11.6 2 11.8 2 12v4c0 .6.4 1 1 1h2" /></svg></div><div className="min-w-0"><p className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase leading-none mb-1 tracking-tighter">Véhicule</p><p className="text-sm font-bold text-slate-700 dark:text-slate-200 truncate">{vehicle.marque} {vehicle.modele} - <span className="text-xs text-slate-500 dark:text-slate-400">{vehicle.immatriculation}</span></p></div></div>)}{mechanic && (<div className="flex items-center gap-3 p-3 bg-blue-50/50 dark:bg-blue-900/10 rounded-2xl border border-blue-100/50 dark:border-blue-500/10"><div className="w-10 h-10 rounded-xl bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center text-blue-600 dark:text-blue-400 font-black text-xs shadow-sm">{mechanic.prenom.charAt(0)}{mechanic.nom.charAt(0)}</div><div className="min-w-0"><p className="text-[9px] font-black text-blue-400 uppercase leading-none mb-1 tracking-tighter">Mécanicien</p><p className="text-sm font-bold text-blue-900 dark:text-blue-300 truncate">{mechanic.prenom} {mechanic.nom}</p></div></div>)}</div><div className="mt-auto pt-6 border-t border-slate-50 dark:border-slate-800 flex items-center justify-between"><div className="flex flex-col"><span className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Durée</span><span className="text-sm font-black text-slate-900 dark:text-white">{app.duree}</span></div><div className="flex gap-2"><button onClick={() => handleEdit(app)} className="p-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-blue-600 dark:text-blue-400 rounded-xl hover:bg-blue-600 hover:text-white dark:hover:bg-blue-600 dark:hover:text-white hover:border-blue-600 dark:hover:border-blue-600 transition-all shadow-sm"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg></button><button onClick={() => setAppointmentToDelete(app)} className="p-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-rose-600 dark:text-rose-400 rounded-xl hover:bg-rose-600 hover:text-white dark:hover:bg-rose-600 dark:hover:text-white hover:border-rose-600 dark:hover:border-rose-600 transition-all shadow-sm"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button></div></div></div>);
          })
        )}
      </div>
