...
const Quotes: React.FC<QuotesProps> = ({ devis, customers, vehicles, settings, userRole, invoices = [], onAdd, onUpdate, onDelete, onAddInvoice, onNavigate, onNotify }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // Le statut ne change pas via ce formulaire en modification, et est forcé à brouillon en création (géré par useEffect 'else')
      ...
      if (editingDevis) {
        ...
        // Note: Le statut n'est plus modifiable ici, donc pas de check de statut
        action = 'modification';
  ...
  const handleChangeStatus = async (newStatus: Devis['statut']) => {
    ...
    try {
        ...
        onNotify('success', 'Statut modifié', `Le devis est maintenant "${newStatus.replace('_', ' ')}".`);
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4" onClick={() => setStatusUpdateQuote(null)}>
            <div className="bg-white dark:bg-slate-900 rounded-[2rem] w-full max-w-sm p-6 shadow-2xl border dark:border-slate-800 animate-in zoom-in duration-200" onClick={e => e.stopPropagation()}>
                ...
                <div className="grid grid-cols-1 gap-2">
                    ...
                    <button onClick={() => handleChangeStatus('en_attente')} className="p-3 rounded-xl bg-amber-50 hover:bg-amber-100 dark:bg-amber-900/20 dark:hover:bg-amber-900/40 text-amber-700 dark:text-amber-400 font-bold text-sm">En Attente</button>
                    <button onClick={() => handleChangeStatus('accepte')} className="p-3 rounded-xl bg-emerald-50 hover:bg-emerald-100 dark:bg-emerald-900/20 dark:hover:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400 font-bold text-sm">Accepté</button>
                    <button onClick={() => handleChangeStatus('refuse')} className="p-3 rounded-xl bg-rose-50 hover:bg-rose-100 dark:bg-rose-900/20 dark:hover:bg-rose-900/40 text-rose-700 dark:text-rose-400 font-bold text-sm">Refusé</button>
                </div>
            </div>
        </div>
      )}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4" onClick={() => setIsModalOpen(false)}>
          <div className="bg-white dark:bg-slate-900 rounded-3xl sm:rounded-[2.5rem] w-full max-w-4xl shadow-2xl relative flex flex-col max-h-[95vh] overflow-hidden border dark:border-slate-800" onClick={(e) => e.stopPropagation()}>
            <div className="p-6 border-b dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/50"><h2 className="text-xl font-black">{editingDevis ? 'Modifier le Devis' : 'Nouveau Devis'}</h2><button onClick={() => setIsModalOpen(false)} className="text-slate-400 hover:text-slate-900 dark:hover:text-white"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M6 18L18 6M6 6l12 12" /></svg></button></div>
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="space-y-1"><DatePicker label="Date" value={formData.date_devis} onChange={d => setFormData({...formData, date_devis: d})} /></div>
              ...
              <div className="flex flex-col md:flex-row gap-8">
                <div className="flex-1 space-y-4"><div className="space-y-1"><label className="text-[10px] font-black uppercase text-slate-400">Notes (Visible sur devis)</label><textarea placeholder="Conditions particulières..." className="w-full p-4 bg-slate-50 dark:bg-slate-800 border dark:border-slate-700 rounded-2xl h-20 text-slate-900 dark:text-white" value={formData.notes} onChange={e => setFormData({...formData, notes: e.target.value})} /></div></div>
                <div className="w-full md:w-72 bg-emerald-50 dark:bg-emerald-900/20 p-6 rounded-[2rem] border border-emerald-100 dark:border-emerald-500/20 space-y-3"><div className="flex justify-between text-sm"><span>Total HT</span><span>{totals.ht.toFixed(2)} €</span></div><div className="flex justify-between text-sm text-emerald-600 dark:text-emerald-400"><span>TVA ({currentModalVat}%)</span><span>{totals.tva.toFixed(2)} €</span></div><div className="pt-3 border-t-2 border-emerald-200 flex justify-between text-xl font-black text-emerald-900 dark:text-emerald-300"><span>Total TTC</span><span>{totals.ttc.toFixed(2)} €</span></div></div>
              </div>
