...
📞 ${settings?.telephone || ''}`
  ...
  const getStatusColor = (s: Devis['statut']) => {
    switch(s) {
      case 'brouillon': return 'bg-slate-100 text-slate-500';
      case 'en_attente': return 'bg-amber-50 text-amber-600';
      case 'accepte': return 'bg-emerald-50 text-emerald-600';
      case 'refuse': return 'bg-rose-50 text-rose-600';
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      {quoteToConvert && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4" onClick={() => !conversionLoading && setQuoteToConvert(null)}>
          <div className="bg-white rounded-[2rem] w-full max-w-sm p-8 shadow-2xl relative animate-in zoom-in duration-300 flex flex-col items-center text-center" onClick={(e) => e.stopPropagation()}>
             ...
             <h3 className="text-xl font-black text-slate-800 mb-2">Convertir en Facture ?</h3>
             <p className="text-slate-500 text-sm mb-6 leading-relaxed">
               Vous allez créer une nouvelle facture basée sur le devis <span className="font-bold text-slate-800">{quoteToConvert.numero_devis}</span>.
             </p>
             <div className="flex flex-col gap-3 w-full">
               ...
               <button onClick={() => setQuoteToConvert(null)} disabled={conversionLoading} className="w-full py-4 bg-white border border-slate-200 text-slate-600 font-black rounded-2xl hover:bg-slate-50 transition-all uppercase tracking-widest text-xs">Annuler</button>
             </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4" onClick={() => setQuoteToDelete(null)}>
          <div className="bg-white rounded-[2rem] w-full max-w-sm p-8 shadow-2xl relative animate-in zoom-in duration-300 flex flex-col" onClick={(e) => e.stopPropagation()}>
             ...
             <div className="flex flex-col gap-3 mt-6">
               ...
               <button onClick={() => setQuoteToDelete(null)} disabled={deleteLoading} className="w-full py-4 bg-white border border-slate-200 text-slate-600 font-black rounded-2xl hover:bg-slate-50 transition-all uppercase tracking-widest text-xs">Annuler</button>
             </div>
          </div>
        </div>
      )}
            ...
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div className="space-y-1">
                  ...
                  <select required className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none font-bold text-sm" value={formData.vehicule_id} onChange={e => setFormData({...formData,vehicule_id: e.target.value})}>
              ...
              <div className="space-y-3">
                ...
                <div className="bg-slate-50 rounded-2xl border border-slate-200 overflow-hidden">
                  ...
                  <button type="button" onClick={addItem} className="w-full py-3 bg-slate-50 hover:bg-slate-100 text-blue-600 font-bold text-xs uppercase tracking-widest transition-colors flex items-center justify-center gap-2 border-t border-slate-200">Ajouter une ligne</button>
                </div>
              </div>
              <div className="flex flex-col md:flex-row gap-8">
                <div className="flex-1 space-y-1">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Notes</label>
                  <textarea className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none font-medium text-sm h-32" value={formData.notes} onChange={e => setFormData({...formData, notes: e.target.value})} />
                </div>
                <div className="w-full md:w-72 bg-slate-50 p-6 rounded-[2rem] border border-slate-200 space-y-3 h-fit">
                  <div className="flex justify-between text-sm text-slate-500 font-medium"><span>Total HT</span><span>{totals.ht.toFixed(2)} €</span></div>
                  <div className="flex justify-between text-sm text-slate-500 font-medium"><span>TVA (20%)</span><span>{totals.tva.toFixed(2)} €</span></div>
                  <div className="pt-3 border-t border-slate-200 flex justify-between text-lg font-black text-slate-800"><span>Total TTC</span><span>{totals.ttc.toFixed(2)} €</span></div>
                </div>
              </div>
      ...
      <div className="space-y-6">
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 font-medium">Créez et suivez vos devis clients.</p>
          </div>
        ...
        <div className="bg-white p-4 rounded-[2rem] border border-slate-100 shadow-sm flex flex-col md:flex-row gap-4">
           <div className="relative flex-1">
              ...
              <input 
                ...
                className="w-full pl-12 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none font-bold text-slate-700 focus:bg-white focus:ring-2 focus:ring-blue-100 transition-all"
              />
           </div>
           <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-1">
              ...
              <button onClick={() => setStatusFilter('en_attente')} className={`px-4 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest whitespace-nowrap border transition-all ${statusFilter === 'en_attente' ? 'bg-amber-100 text-amber-700 border-amber-200' : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-50'}`}>En Attente</button>
              <button onClick={() => setStatusFilter('accepte')} className={`px-4 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest whitespace-nowrap border transition-all ${statusFilter === 'accepte' ? 'bg-emerald-100 text-emerald-700 border-emerald-200' : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-50'}`}>Accepté</button>
              <button onClick={() => setStatusFilter('refuse')} className={`px-4 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest whitespace-nowrap border transition-all ${statusFilter === 'refuse' ? 'bg-rose-100 text-rose-700 border-rose-200' : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-50'}`}>Refusé</button>
           </div>
        </div>
      </div>
              ...
              {filteredDevis.length === 0 ? (
                <tr>
                  <td colSpan={6} className="py-20 text-center">
                    <p className="text-slate-400 font-bold italic">Aucun devis trouvé</p>
                  </td>
                </tr>
              ) : (
                filteredDevis.map((d) => {
                  ...
                  return (
                    <tr key={d.id} className="hover:bg-slate-50 transition-all group">
                      ...
                      <td className="px-6 py-5 text-right flex justify-end gap-2">
                        ...
                        {isConverted ? (
                            <div className="p-2 text-emerald-600 bg-emerald-50 rounded-lg border border-emerald-100 flex items-center justify-center cursor-help" title="Converti"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7" /></svg></div>
                        ) : (
