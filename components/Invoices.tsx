          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl sm:rounded-[2.5rem] w-full max-w-4xl shadow-2xl relative flex flex-col max-h-[95vh] overflow-hidden border dark:border-slate-800" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className="flex flex-col md:flex-row gap-8">
                ...
                <div className="w-full md:w-72 bg-emerald-50 dark:bg-emerald-900/20 p-6 rounded-[2rem] border border-emerald-100 dark:border-emerald-500/20 space-y-3"><div className="flex justify-between text-sm"><span>{t('quotes.total_ht')}</span><span>{totals.ht.toFixed(2)} €</span></div><div className="flex justify-between text-sm"><span>{t('quotes.total_ttc')}</span><span>{totals.ttc.toFixed(2)} €</span></div>{totals.acompte > 0 && <div className="flex justify-between text-sm font-bold border-t pt-2"><span>{t('invoices.form_deposit')}</span><span>- {totals.acompte.toFixed(2)} €</span></div>}<div className="pt-3 border-t-2 border-emerald-200 flex justify-between text-xl font-black text-emerald-900 dark:text-emerald-300"><span>{t('invoices.form_to_pay')}</span><span>{totals.rest.toFixed(2)} €</span></div></div>
              </div>
