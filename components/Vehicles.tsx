...
const Vehicles: React.FC<VehiclesProps> = ({ vehicles, customers, appointments, invoices, onAdd, onUpdate, onDelete }) => {
  ...
  const getStatusBadge = (status: string) => {
    switch(status) {
      case 'termine': case 'payee': return 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400';
      case 'en_cours': case 'en_attente': return 'bg-blue-100 text-blue-700 dark:bg-blue-500/10 dark:text-blue-400';
      case 'non_payee': return 'bg-amber-100 text-amber-700 dark:bg-amber-500/10 dark:text-amber-400';
      case 'annule': return 'bg-rose-100 text-rose-700 dark:bg-rose-500/10 dark:text-rose-400';
      ...
      {historyVehicle && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4" onClick={() => setHistoryVehicle(null)}>
          <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] w-full max-w-2xl shadow-2xl relative animate-in zoom-in duration-300 flex flex-col max-h-[85vh] overflow-hidden border dark:border-slate-800" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex-1 overflow-y-auto p-6 bg-slate-50 dark:bg-slate-950 scrollbar-hide">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {filteredVehicles.length === 0 ? (
          ...
          filteredVehicles.map((v) => {
            ...
            return (
              <div key={v.id} className="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-slate-800 p-6 flex flex-col hover:shadow-xl hover:-translate-y-1 transition-all group overflow-hidden relative animate-in zoom-in duration-300">
                {/* En-tête Carte */}
                <div className="flex justify-between items-start mb-6">
                  <div className="w-12 h-12 bg-blue-50 dark:bg-blue-900/10 text-blue-600 dark:text-blue-400 rounded-2xl flex items-center justify-center group-hover:bg-blue-600 group-hover:text-white dark:group-hover:bg-blue-600 dark:group-hover:text-white transition-all duration-300 shadow-sm">
                    <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 17h2c.6 0 1-.4 1-1v-3c0-.9-.7-1.7-1.5-1.9C18.7 10.6 16 10 16 10s-1.3-1.4-2.2-2.3c-.5-.4-1.1-.7-1.8-.7H5c-.6 0-1.1.4-1.4.9l-1.4 2.9C2.1 11.6 2 11.8 2 12v4c0 .6.4 1 1 1h2m2 0h10" /></svg>
                  </div>
                  <span className="px-3 py-1.5 bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-black text-[10px] rounded-xl tracking-wider shadow-md">
                ...
                <div className="mt-auto pt-4 border-t border-slate-50 dark:border-slate-800 flex items-center justify-between">
                  <div className="flex items-center gap-2 overflow-hidden">
                    <div className="w-8 h-8 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-lg flex items-center justify-center text-slate-400 text-[10px] font-black shadow-sm group-hover:text-blue-600 transition-colors shrink-0">
                      {owner?.nom?.charAt(0) || '?'}
                    </div>
